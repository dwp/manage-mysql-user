...
def update_password_parameter(password, parameter_name):
    ...
    try:
        ssm.put_parameter(
            Name=parameter_name, Type="SecureString", Value=password, Overwrite=True
        )
...
def get_master_password():
    ...
    return ssm.get_parameter(
        Name=os.environ["RDS_MASTER_PASSWORD_PARAMETER_NAME"], WithDecryption=True
    )["Parameter"]["Value"]
...
def execute_statement(sql):
    connection = get_connection(
        get_mysql_password(os.environ["RDS_MASTER_PASSWORD_PARAMETER_NAME"])
    )
...
def handler(event, context):
    ...
    test_result = test_connection(
        mysql_user_username, mysql_user_password_parameter_name
    )
    if test_result:
        logger.info(
            f"Password rotation complete: MySQL user {mysql_user_username} succesfully logged in using password from SSM parameter {mysql_user_password_parameter_name}"
        )
    else:
        raise ValueError(
            f"Password rotation failed: MySQL user {mysql_user_username} failed to login using password from SSM parameter {mysql_user_password_parameter_name}"
        )
